...
# <start>
# The first and second instructions must be updated to store PMU base address in x1.
...
# The sixth instruction must be updated to store the size of `COUNTER_BUNDLE` in bytes.
80000014:       addi x3,x0,16
# The seventh instruction contains the incremental value that is written to each register.
# Set up loop variables. Skip the `PMU_BUNDLE`.
80000018:       add x4,x0,x0
8000001c:       add x5,x1,x0
80000020:       addi x6,x0,271
# Loop starts.
80000024:       sw x6,0(x5)
80000028:       addi x6,x6,271
8000002c:       sw x6,4(x5)
80000030:       addi x6,x6,271
80000034:       sw x6,8(x5)
80000038:       addi x6,x6,271
8000003c:       sw x6,12(x5)
80000040:       addi x6,x6,271
# Update loop variables.
80000044:       addi x4,x4,1
80000048:       add x5,x5,x3
# Check if `x4 == x2` (`NUM_COUNTER`). If yes, go to end of program. Otherwise, go to <start+0x1c>.
8000004c:       bne x4,x2,-40
# Program ends by writing a 101 to the PMU DSPM (`x7`) and then loop forever.
80000050:       addi x8,x0,101
80000054:       sw x8,0(x7)
80000058:       add x0,x0,x0
8000005c:       beq x0,x0,-4
