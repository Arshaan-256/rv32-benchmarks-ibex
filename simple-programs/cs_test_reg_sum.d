...
0x00000098:         mul x16,x27,x4
0x0000009c:         mulhu x17,x27,x4
...
0x000000a0:         bltu x11,x9,$REGULATE
0x000000a4:         bne x11,x9,$NO_REGULATE
0x000000a8:         bltu x10,x8,$REGULATE
$NO_REGULATE:       0x000000ac
0x000000ac:         addi x9,x0,0
0x000000b0:         beq x0,x0,$INNER_LOOP
$REGULATE:          0x000000b4
0x000000b4:         addi x9,x0,1
...
# *******************************************************************
# Load loop variable.
$INNER_LOOP:        0x000000b8
0x000000b8:         addi x2,x2,1
# x3 = x3+1, x10 = cnt[x3], LLC read accesses by non-CUA
0x000000bc:         addi x3,x3,1
0x000000c0:         cnt.rd x10,x3
# x3 = x3+1, x11 = cnt[x3], LLC read misses by non-CUA
0x000000c4:         addi x3,x3,1
0x000000c8:         cnt.rd x11,x3
# x3 = x3+1, x12 = cnt[x3], LLC write accesses
0x000000cc:         addi x3,x3,1
0x000000d0:         cnt.rd x12,x3
# x3 = x3+1, x13 = cnt[x3], LLC write misses
0x000000d4:         addi x3,x3,1
0x000000d8:         cnt.rd x13,x3
# x10 = x10 - x11, LLC read hits by non-CUA
0x000000dc:         sub x10,x10,x11
# x12 = x12 - x13, LLC write hits by non-CUA
0x000000e0:         sub x12,x12,x13
#
#
...
0x000000e4:         mul x4,x10,x23
0x000000e8:         mulhu x5,x10,x23
# Read Misses (x11, x24)
# x7, x6  = x11 * x24
0x000000ec:         mul x6,x11,x24
0x000000f0:         mulhu x7,x11,x24
# x5 = x5 + x7 ; x4 = x4 + x6
0x000000f4:         add x5,x5,x7
0x000000f8:         add x4,x4,x6
# Check carry logic: Dest <= Dest + Src, if Dest < Src then there was carry OR if Dest >= Src then there was no carry.
0x000000fc:         bgeu x4,x6,$NO_CARRY_RM
0x00000100:         addi x5,x5,1
#
...
$NO_CARRY_RM:       0x00000104
0x00000104:         bltu x17,x5,$HALT_DECISION
0x00000108:         bne x17,x5,$CONTINUE_WH
0x0000010c:         bltu x16,x4,$HALT_DECISION
#
#
# Write Hits (x12, x25)
# x7, x6  = x12 * x25
$CONTINUE_WH:       0x00000110
0x00000110:         mul x6,x12,x25
0x00000114:         mulhu x7,x12,x25
# x5 = x5 + x7 ; x4 = x4 + x6
0x00000118:         add x5,x5,x7
0x0000011c:         add x4,x4,x6
# Check carry logic: Dest <= Dest + Src, if Dest < Src then there was carry OR if Dest >= Src then there was no carry.
0x00000120:         bgeu x4,x6,$NO_CARRY_WH
0x00000124:         addi x5,x5,1
#
# Check if WC_Delay > Acceptable Delay.
# If Uaccept <  Uwc => Halt core ; else continue
# If Uaccept != Uwc => Don't halt core ; else continue
# If Laccept <  Lwc => Halt core ; else continue
# Accept_delay = {x17,x16} and WC_delay = {x5,x4}
$NO_CARRY_WH:       0x00000128
0x00000128:         bltu x17,x5,$HALT_DECISION
0x0000012c:         bne x17,x5,$CONTINUE_WM
0x00000130:         bltu x16,x4,$HALT_DECISION
#
#
# Write Misses (x13, x26)
# x7, x6  = x13 * x26
$CONTINUE_WM:       0x00000134
0x00000134:         mul x6,x13,x26
0x00000138:         mulhu x7,x13,x26
# x5 = x5 + x7 ; x4 = x4 + x6
0x0000013c:         add x5,x5,x7
0x00000140:         add x4,x4,x6
# Check carry logic: Dest <= Dest + Src, if Dest < Src then there was carry OR if Dest >= Src then there was no carry.
0x00000144:         bgeu x4,x6,$NO_CARRY_WM
0x00000148:         addi x5,x5,1
#
# Check if WC_Delay > Acceptable Delay.
# If Uaccept <  Uwc => Halt core ; else continue
# If Uaccept != Uwc => Don't halt core ; else continue
# If Laccept <  Lwc => Halt core ; else continue
# Accept_delay = {x17,x16} and WC_delay = {x5,x4}
$NO_CARRY_WM:       0x0000014c
0x0000014c:         bltu x17,x5,$HALT_DECISION
0x00000150:         bne x17,x5,$NO_HALT_DECISION
0x00000154:         bltu x16,x4,$HALT_DECISION
#
#
# x5 (Reuse) = 1, if acceptable-delay < WC-delay else 0.
$NO_HALT_DECISION:  0x00000158
0x00000158:         addi x5,x0,0
0x0000015c:         beq x0,x0,$STATUS
$HALT_DECISION:     0x00000160
0x00000160:         addi x5,x0,1
#
$STATUS:            0x00000164
# Mask = 1 << Core Number
# x21  = x1 << x2
0x00000164:         sll x19,x1,x2
# x21 = (x20 & x19) (BitVector & Mask)
0x00000168:         and x21,x20,x19
# x21 = x21 >> x2
# CoreHalted = (BitVector & Mask) >> Core Number
0x0000016c:         srl x21,x21,x2
#
# Resume core = CoreHalted & (WC-delay =< acceptable-delay)
# x22         = x21        & x6 (=!x5) (Reuse) 
# If 1, go to Resume Function.
0x00000170:         xori x6,x5,1
0x00000174:         and x22,x21,x6
0x00000178:         beq x22,x1,$RESUME_CORE
#
# Halt core = Regulate & !CoreHalted & (acceptable-delay < WC-delay)
# x22       = x9       & !x21        & x5   
# x21[0] = !x21[0]
# If 1, go to Halt Function. (000000ec)
0x0000017c:         xori x21,x21,1
0x00000180:         and x22,x9,x21
0x00000184:         and x22,x22,x5
0x00000188:         beq x22,x1,$HALT_CORE
#
# ****************************************
...
$UPDATE:            0x0000018c
0x0000018c:         bne x2,x31,$INNER_LOOP
# Go to OuterLoop. (00000030)
# This is reset by CVA6.
0x00000190:         sw x1,36(x30)
0x00000194:         beq x0,x0,$OUTER_LOOP
#
# DSPM Addresses.
# ***************
# 0x040 for regulation time of Core 1.  
...
$HALT_CORE:         0x00000198
0x00000198:         or x20,x20,x19
# Store core_status at DSPM_BASE_ADDR + 0x20
0x0000019c:         sw x20,32(x30)
#
...
0x000001a0:         addi x4,x2,5
0x000001a4:         sll x4,x1,x4
0x000001a8:         add x4,x30,x4
# x6 = Upper bits of PMU Timer.
# x5 = Lower bits of PMU Timer.
0x000001ac:         lw x5,0(x18)
0x000001b0:         lw x6,4(x18)
# Store timestamp for current halt decision.
0x000001b4:         sw x5,8(x4)
0x000001b8:         sw x6,12(x4)
# Go to Update Function. (000000e0)
0x000001bc:         beq x0,x0,$UPDATE
# ***************
# Resume Function
# ***************
# Debug Resume
# 00000000: sw x2,8(x29)
$RESUME_CORE:       0x000001c0
0x000001c0:         xori x19,x19,-1
0x000001c4:         and x20,x20,x19
# Store core_status at DSPM_BASE_ADDR + 0x20
0x000001c8:         sw x20,32(x30)
#
# Regulation Time Calculator.
# For core 1, 0x040, the offset is 6.
# For core 2, 0x080, the offset is 7.
