...
0x0000007c:     bltu x11,x8,$REGULATE
0x00000080:     bne x11,x8,$NO_REGULATE
0x00000084:     bltu x10,x11,$REGULATE
$NO_REGULATE:0x00000088
0x00000088:     addi x9,x0,0
0x0000008c:     beq x0,x0,$INNER_LOOP
$REGULATE:0x00000090
0x00000090:     addi x9,x0,1
...
0x000000c0:     mul x4,x10,x23
0x000000c0:     mulhu x5,x10,x23
# Read Misses (x11, x24)
# x7, x6  = x11 * x24
0x000000c0:     mul x6,x11,x24
0x000000c0:     mulhu x7,x11,x24
# x5 = x5 + x7 ; x4 = x4 + x6
0x000000c0:     add x5,x5,x7
0x000000c0:     add x4,x4,x6
# Check carry logic: Dest <= Dest + Src, if Dest < Src then there was carry OR if Dest >= Src then there was no carry.
0x0000006c:     bgeu x4,x6,$NO_CARRY_RM
0x0000006c:     addi x5,x5,1
#
...
$NO_CARRY_RM:0x0000006c
0x0000007c:     bltu x17,x5,$HALT
0x00000080:     bne x17,x5,$CONTINUE_WH
0x00000084:     bltu x16,x4,$HALT
#
#
# Write Hits (x12, x25)
# x7, x6  = x12 * x25
$CONTINUE_WH:0x000000c0
0x000000c0:     mul x6,x12,x25
0x000000c0:     mulhu x7,x12,x25
# x5 = x5 + x7 ; x4 = x4 + x6
0x000000c0:     add x5,x5,x7
0x000000c0:     add x4,x4,x6
# Check carry logic: Dest <= Dest + Src, if Dest < Src then there was carry OR if Dest >= Src then there was no carry.
0x0000006c:     bgeu x4,x6,$NO_CARRY_WH
0x0000006c:     addi x5,x5,1
#
# Check if WC_Delay > Acceptable Delay.
# If Uaccept <  Uwc => Halt core ; else continue
...
# If Laccept <  Lwc => Halt core ; else continue
# Accept_delay = {x17,x16} and WC_delay = {x5,x4}
$NO_CARRY_WH:0x0000006c
0x0000007c:     bltu x17,x5,$HALT
0x00000080:     bne x17,x5,$CONTINUE_WM
0x00000084:     bltu x16,x4,$HALT
#
#
# Write Misses (x13, x26)
# x7, x6  = x13 * x26
$CONTINUE_WM:0x00000084
0x000000c0:     mul x6,x12,x25
0x000000c0:     mulhu x7,x12,x25
# x5 = x5 + x7 ; x4 = x4 + x6
0x000000c0:     add x5,x5,x7
0x000000c0:     add x4,x4,x6
# Check carry logic: Dest <= Dest + Src, if Dest < Src then there was carry OR if Dest >= Src then there was no carry.
0x0000006c:     bgeu x4,x6,$NO_CARRY_WM
0x0000006c:     addi x5,x5,1
#
# Check if WC_Delay > Acceptable Delay.
# If Uaccept <  Uwc => Halt core ; else continue
...
# If Laccept <  Lwc => Halt core ; else continue
# Accept_delay = {x17,x16} and WC_delay = {x5,x4}
$NO_CARRY_WM:0x0000006c
0x0000007c:     bltu x17,x5,$HALT
0x00000080:     bne x17,x5,$NO_HALT
0x00000084:     bltu x16,x4,$HALT
#
#
# x17 = 1, if acceptable-delay < WC-delay else 0.
$NO_HALT:0x000000e0
0x000000e0:     add x17,x0,0
0x000000e0:     beq x0,x0,$STATUS
$HALT:0x000000e0
0x000000e0:     add x17,x0,1
#
$STATUS:0x000000e0
# Mask = 1 << Core Number# x6  - 
# x21  = x1 << x2
0x000000e4:     sll x19,x1,x2
# x21 = (x20 & x19) (BitVector & Mask)
0x000000e8:     and x21,x20,x19
# x21 = x21 >> x2
# CoreHalted = (BitVector & Mask) >> Core Number
0x000000ec:     srl x21,x21,x2
#
# Resume core = CoreHalted & (WC-delay =< acceptable-delay)
# x22         = x21        & x18 (=!x17)
# If 1, go to Resume Function.
0x000000f0:     xori x18,x17,1
0x000000f4:     and x22,x21,x18
0x000000f8:     beq x22,x1,56
#
# Halt core = Regulate & !CoreHalted & (acceptable-delay < WC-delay)
# x22       = x9       & !x21        & x17   
# x21[0] = !x21[0]
# If 1, go to Halt Function. (000000ec)
0x000000fc:     xori x21,x21,1
0x00000100:     and x22,x9,x21
0x00000104:     and x22,x22,x17
0x00000108:     beq x22,x1,16
#
# ****************************************
# Update: Loop Variable and run next epoch
# ****************************************
# If x2 < x31: go to InnerLoop. (00000068)
0x0000010c:     blt x2,x31,-120
# Go to OuterLoop. (00000030)
# This is reset by CVA6.
0x00000110:     sw x1,28(x30)
0x00000114:     beq x0,x0,-184
#
# *************
# Halt Function
# *************
# 00000000: sw x2,0(x29)
0x00000118:     add x0,x0,x0
0x0000011c:     add x0,x0,x0
0x00000120:     sw x20,24(x30)
0x00000124:     add x0,x0,x0
0x00000128:     or x20,x20,x19
# Go to Update Function. (000000e0)
0x0000012c:     beq x0,x0,-32
# ***************
# Resume Function
# ***************
# 00000000: sw x2,8(x29)
0x00000130:     xori x19,x19,-1
0x00000134:     and x20,x20,x19
0x00000138:     sw x20,24(x30)
# Go to Update Function. (000000e0)
0x0000013c:     beq x0,x0,-48
